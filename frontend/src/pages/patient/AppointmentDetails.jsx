  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] -mt-8 -mx-8 relative overflow-hidden bg-gray-50">
      ...
      <div className="h-16 border-b border-gray-200 bg-white flex items-center justify-between px-8 shrink-0">
        <button onClick={() => navigate('/patient/appointments')} className="flex items-center text-gray-600 hover:text-gray-900 text-sm font-medium">
        ...
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr,380px] gap-6 items-start h-full pb-10">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-4 flex flex-col md:flex-row items-start md:items-center justify-between shadow-sm gap-4">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-indigo-600 text-xl font-bold shrink-0 overflow-hidden shadow-inner">
                  {appointment.doctorPic ? <img src={appointment.doctorPic} alt={appointment.doctorName} className="w-full h-full object-cover" /> : appointment.doctorName?.[0]}
                </div>
                <div>
                  <h2 className="text-[18px] font-bold text-gray-900 tracking-tight">{appointment.doctorName}</h2>
                  <p className="text-sm text-gray-500 font-medium mb-1">{appointment.specialty}</p>
                  <div className="flex items-center text-sm text-gray-500 font-medium gap-3">
                    <span className="flex items-center text-yellow-500">
                      <Star className="w-4 h-4 fill-current mr-1" />
                      4.9 <span className="text-gray-400 font-normal ml-1">(127 reviews)</span>
                    </span>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm flex flex-col">
              ...
              {appointment.mode === 'ONLINE' ? (
                <div className="bg-[#111827] rounded-[20px] p-6 flex flex-col items-center justify-center text-center shadow-md relative overflow-hidden">
                  ...
                  <p className="text-gray-400 text-[13px] mb-6 font-medium">Your consultation will begin here</p>
                  
                  <button 
                    onClick={() => window.open(appointment.linkOrAddress || 'https://meet.google.com/', '_blank')}
                ...
                <div className="bg-[#EFF6FF] rounded-[20px] p-6 flex flex-col items-center text-center border border-[#BFDBFE]">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 w-full max-w-[200px] justify-center">
                     <button className="flex-1 bg-[#2563EB] hover:bg-[#1D4ED8] text-white py-2.5 rounded-[10px] font-bold text-[14px] flex items-center justify-center transition-colors shadow-sm">
                        <Map className="w-5 h-5 mr-2" /> Get Directions
                     </button>
                  </div>
                </div>
              )}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 flex flex-col shadow-sm h-[calc(100vh-10rem)] lg:h-[calc(100vh-8rem)] lg:sticky lg:top-6 min-h-[400px]">
            ...
            <div className="p-5 border-b border-gray-100 flex items-center justify-between shrink-0">
              <div>
                <h3 className="font-bold text-gray-900 text-lg">Chat with {appointment.doctorName}</h3>
                ...
                messages.map((msg, index) => {
                  ...
                  return (
                    <div key={msg.id || index} className="flex flex-col gap-2">
                      <div className={`${isPatient ? 'bg-[#2563EB] text-white self-end rounded-tr-sm shadow-sm' : 'bg-gray-100 text-gray-700 self-start rounded-tl-sm'} px-4 py-3 rounded-2xl text-[15px] max-w-[85%] font-medium leading-relaxed`}>
                        {msg.text}
                      </div>
                      <span className={`text-[11px] text-gray-400 font-medium ${isPatient ? 'mr-1 self-end' : 'ml-1'}`}>
                        {timeString}
                      </span>
                    </div>
                  );
